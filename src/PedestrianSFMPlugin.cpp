...
void PedestrianSFMPlugin::Reset()
{
  ...
  if (this->sdf->HasElement("trajectory"))
  {
    ...
    sdf::ElementPtr modelElem = this->sdf->GetElement("trajectory")->GetElement("waypoint");
    while (modelElem)
    {
...
void PedestrianSFMPlugin::HandleObstacles()
{
  ...
  {
    physics::ModelPtr model = this->world->ModelByIndex(i);  // GetModel(i);
    if (std::find(this->ignoreModels.begin(), this->ignoreModels.end(), model->GetName()) ==
        this->ignoreModels.end())
    {
      ...
      {
        //printf("Dist center: %.2f, dist intersect: %.2f\n", dist1, dist2);
        //printf("instersect ok: %i, PosDist: %.2f, modelDist: %.2f, modelDist1: %.2f\n",
        //       (int)std::get<0>(intersect), std::get<2>(intersect).Length(), modelDist, modelDist1);
        minDist = modelDist;
        closest_obs = offset;
      }
      //}
...
void PedestrianSFMPlugin::HandlePedestrians()
{
  ...
  //printf("Actor %i has detected %i actors!\n", this->actor->GetId(), (int)this->otherActors.size());
}
